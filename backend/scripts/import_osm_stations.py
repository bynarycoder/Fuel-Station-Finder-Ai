...
# When executed as ``python backend/scripts/import_osm_stations.py`` from the
# repository root, Python adds ``backend/scripts`` to sys.path, not ``backend``.
# Ensure ``backend`` is importable as a top-level package for ``app.*`` imports.
...
from app.services.station_import import (  # noqa: E402
    ImportSummary,
    import_stations,
    parse_records,
)
...
DATASET_PATH = (
    REPOSITORY_ROOT / "scripts" / "data" / "output" / "nigeria_osm_fuel_stations.json"
)
...
def load_dataset(path: Path = DATASET_PATH) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise DatasetLoadError(
            "Dataset is not valid JSON " f"(line {exc.lineno}, column {exc.colno})."
        ) from exc
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    return argparse.ArgumentParser(
        prog="python backend/scripts/import_osm_stations.py",
        description=(
